	...
	public void testNetworkLinks() {
		...
		for (Link l : links) {
			...
			if (((IntValue) l.getAttribute("id").get(0)).getValue() == 0) {
				assertFalse(((BooleanValue) l.getAttribute("directed").get(0))
						.getValue());
				assertEquals(
						((IntValue) l.getAttribute("tail").get(0)).getValue(),
						1);
				assertEquals(
						((IntValue) l.getAttribute("head").get(0)).getValue(),
						0);
				assertEquals(
						((IntValue) l.getAttribute("type").get(0)).getValue(),
						0);
				assertEquals(
				...
				assertEquals(
						((LinkProbaDist) l.getAttribute(
								"probabilityDistribution").get(0)).getMoments()
								.get(0).getValue(), 1.1, 0);
				assertEquals(
						((LinkProbaDist) l.getAttribute(
								"probabilityDistribution").get(0)).getMoments()
								.get(1).getValue(), 2.1, 0);
				assertEquals(
						((LinkProbaDist) l.getAttribute(
								"probabilityDistribution").get(0)).getName(),
						"normal");
				assertEquals(
						((LinkProbaDist) l.getAttribute(
								"probabilityDistribution").get(0)).getMoments()
								.get(0).getName(), "String1");
				assertEquals(
						((LinkProbaDist) l.getAttribute(
								"probabilityDistribution").get(0)).getMoments()
								.get(1).getName(), "String2");
	...
	public void testFleetVehicle() {
		for (Vehicle v : Instance.getFleet()) {
			...
			if (((IntValue) v.getAttribute("type").get(0)).getValue() == 0) {
				assertEquals(
						((DoubleValue) v.getAttribute("fixedCost").get(0))
								.getValue(),
						3.1, 0);
				assertEquals(
						((DoubleValue) v.getAttribute("fixedCost").get(1))
								.getValue(),
						3.14, 0);
				assertEquals(
						((DoubleValue) v.getAttribute("variableCost").get(0))
								.getValue(),
						3.141, 0);
				assertEquals(
						((DoubleValue) v.getAttribute("variableCost").get(1))
								.getValue(),
						3.1415, 0);
				assertEquals(
						((IntValue) v.getAttribute("number").get(0)).getValue(),
						0);
				assertEquals(((IntValue) v.getAttribute("nodeTypesCompatible")
						.get(0)).getValue(), 0);
				assertEquals(((IntValue) v.getAttribute("nodeTypesCompatible")
						.get(1)).getValue(), 1);
				assertEquals(
						((DoubleValue) v.getAttribute("speedProfile").get(0))
								.getValue(),
						3.14159, 0);
				assertEquals(((DoubleValue) v.getAttribute("maxTravelDistance")
						.get(0)).getValue(), 34, 0);
				...
				assertEquals(((DoubleValue) v.getAttribute("wLPMaxWorkTime")
						.get(0)).getValue(), 32, 0);
				assertFalse(((BooleanValue) v.getAttribute(
				...
				assertEquals(
						((IntValue) v.getAttribute("departureNode").get(0))
								.getValue(),
						0);
				assertEquals(
						...
						1);
				assertEquals(
						((SkillAndTool) v.getAttribute("skill").get(0)).getId(),
						0);
				assertEquals(
						...
						1);
				assertEquals(
						((SkillAndTool) v.getAttribute("skill").get(1)).getId(),
						1);
				assertEquals(
				...
				//
				assertEquals(
						((SkillAndTool) v.getAttribute("tool").get(0)).getId(),
						0);
				assertEquals(
						...
						3);
				assertEquals(
						((SkillAndTool) v.getAttribute("tool").get(1)).getId(),
						1);
				assertEquals(
	...
	public void testRequests() {
		for (Request r : Instance.getRequests()) {
			...
			if (r.getId() == 0) {
				...
				assertEquals(
						((IntValue) r.getAttribute("type").get(0)).getValue(),
						0);
				assertEquals(
						((IntValue) r.getAttribute("link").get(0)).getValue(),
						0);
				assertEquals(
						((IntValue) r.getAttribute("node").get(0)).getValue(),
						0);
				assertEquals(
						((TimeWindow) r.getAttribute("tw").get(0)).getBegin(),
						53, 0);
				assertEquals(
				...
				assertTrue(((TimeWindow) r.getAttribute("tw").get(0))
						.isHardEnd());
				assertEquals(
						((TimeWindow) r.getAttribute("tw").get(0)).getPeriod(),
						0);

				//
				assertEquals(
						((TimeWindow) r.getAttribute("tw").get(1)).getBegin(),
						56, 0);
				assertEquals(
				...
				assertTrue(((TimeWindow) r.getAttribute("tw").get(1))
						.isHardEnd());
				assertEquals(
						((TimeWindow) r.getAttribute("tw").get(1)).getPeriod(),
						1);

				assertFalse(((DemandProbaDist) r.getAttribute("demand").get(0))
						.isSplittable());
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
								.getType(),
						0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
				...
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
								.getMoments().get(0).getValue(), 3.1415, 0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
								.getMoments().get(0).getName(), "String");
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
								.getMoments().get(1).getValue(), 3.1410, 0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
								.getMoments().get(1).getName(), "String");
				//
				assertFalse(((DemandProbaDist) r.getAttribute("demand").get(1))
						.isSplittable());
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
								.getType(),
						0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
				...
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
								.getMoments().get(1).getValue(), 3.14, 0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
								.getMoments().get(1).getName(), "String");
				...
				assertEquals(((RequestProbaDist) r.getAttribute("serviceTime")
						.get(0)).getName(), "normal");
				assertEquals(((RequestProbaDist) r.getAttribute("serviceTime")
						.get(0)).getMoments().get(0).getValue(), 3.141, 0);
				assertEquals(((RequestProbaDist) r.getAttribute("serviceTime")
						.get(0)).getMoments().get(0).getName(), "String");
				assertEquals(((RequestProbaDist) r.getAttribute("serviceTime")
						.get(0)).getMoments().get(1).getValue(), 3.14, 0);
				assertEquals(((RequestProbaDist) r.getAttribute("serviceTime")
				...
				assertEquals(
						...
						1);
				/*
				 * assertEquals( ((IntValue) r.getAttribute("successor").get(1))
				 * .getValue(), 0);
				 */

				assertEquals(
						((SkillAndTool) r.getAttribute("skill").get(0)).getId(),
						0);
				assertEquals(
						...
						1);
				assertEquals(
						((SkillAndTool) r.getAttribute("skill").get(1)).getId(),
						1);
				assertEquals(
				...
				//
				assertEquals(
						((SkillAndTool) r.getAttribute("tool").get(1)).getId(),
						1);
				assertEquals(
						...
						4);
				assertEquals(
						((SkillAndTool) r.getAttribute("tool").get(0)).getId(),
						2);
				assertEquals(
			...
			if (r.getId() == 1) {
				...
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
								.getMoments().get(1).getValue(), 3.1415, 0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(0))
								.getMoments().get(1).getName(), "String");
				//
				...
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
								.getMoments().get(0).getValue(), 3.141, 0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
								.getMoments().get(0).getName(), "String");
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
								.getMoments().get(1).getValue(), 3.1, 0);
				assertEquals(
						((DemandProbaDist) r.getAttribute("demand").get(1))
								.getMoments().get(1).getName(), "String");
